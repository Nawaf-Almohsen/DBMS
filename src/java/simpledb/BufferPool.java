...
public class BufferPool {
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		// check if we have the read Permissions and if the page is not locked
		if ((perm.toString().equalsIgnoreCase("READ_ONLY") || perm.toString().equalsIgnoreCase("READ_WRITE"))
				&& holdsLock(tid, pid)) {

			Queue<Page> temp = new LinkedList<Page>();
			Page p = null;
			boolean foundInBufferFlag = false;
			for (int i = 0; i < queue.size(); i++) {
				p = queue.remove();
				temp.add(p);
				if (p.getId() == pid) {
					foundInBufferFlag = true;
					break;
				}
			}
			for (int i = 0; i < queue.size(); i++) {
			...
			if (foundInBufferFlag) {
				return p;
			}
			perm.toString();
/////////////////////////////////////////////////////////////////////////////////////
			//// /////below code if we did not found the page in BufferPool //////
	...
	public void insertTuple(TransactionId tid, int tableId, Tuple t)
		...
		/*
		 * get heapfile from catalog insert tuple using heapfile method Mark returned
		 * pages as dirty and place them in the buffer
		 */
	}
